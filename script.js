...
// 六經路線理由行 (2026-07-12): explanation.channelRoute is set by x4-adapter.js
// only when the formula's rank came from the 六經 route (max() winner), e.g.
...
// 主訴相符行 (2026-07-13): rendered when the chief bonus lifted this card.
function buildChiefMatchHtml(explanation) {
    if (!explanation || !explanation.chiefMatch) return '';
    return '<div><strong><i class="fa-solid fa-bullseye"></i> 主訴相符：</strong>' + escapeHtml(explanation.chiefMatch) + '</div>';
}

// 主證相符行 (2026-07-16): rendered when the cardinal bonus lifted this card.
function buildCardinalMatchHtml(explanation) {
    if (!explanation || !explanation.cardinalMatch) return '';
    return '<div><strong><i class="fa-solid fa-star"></i> 主證相符：</strong>' + escapeHtml(explanation.cardinalMatch) + '</div>';
}
