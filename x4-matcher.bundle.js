...
//
// **evidence 否決**：它把大塚的第1 換前3（w 越大第1 越好、前3 越差），臨床要 w≈0.6
// 才追平並小勝一案，但同時萩野第1 −1。三軸互相拉扯，是換一組 trade-off 不是解決。
//
// **softcap γ=2 上線**（2026-07-31）。總數只看得到「大塚第1 +2、前3 −1」，逐案 diff
// 才看得出這個交換的形狀：
//   萩野：改善 1（#67 痤瘡 5→4）、退步 0
//   大塚：改善 5、退步 3
//     ＋ 頭痛→加味逍遙散 2→1、頭痛→桂枝人參湯 2→1   ← 兩個第1
//     ＋ 目眩→黃連解毒湯 3→2、目眩→三黃瀉心湯 5→4、下痢→柴胡桂枝湯 無→5
//     − 精神異常→大承氣湯 3→無（唯一真的掉出前五）
//     − 精神異常→柴胡加龍骨牡蠣湯 2→3、浮腫→分消湯 4→5（名次微調，仍在榜上）
//   臨床教科書 22 案：**逐案位元相同**。
// 8 個改善對 3 個退步、只有一個掉出前五，而金標準完全不動——收。
const KEY_SCORE_MODEL = "softcap";
// 未解釋主症的不利權重（只在 evidence 模型下有作用）。0 ＝ 完全不管方多大（純 volume）。
const MISS_WEIGHT = 0.5;
// softcap 的曲率。1 ＝ 線性內插；>1 ＝ 要 recall 夠高才給封頂（更接近既有的二元行為）。
// 掃 γ∈{1, 1.5, 2, 3, 4}：大塚 第1 全部 40-41，但 前5 只有 γ=2 保住 82（γ=1/1.5 掉到 81）。
// γ≥3 起第1 又回到 40。取 2。
...
// 在與症狀清單可能衝突時，舌象決定方向。
// 實現方式：識別患者舌象，給舌象匹配的方劑加分，然後重新排序。
// 加分幅度：0.30（類似於一個中等關鍵症狀的貢獻）
// 舌象優先級：加分版（2026-07-25 上線）**2026-07-28 量測後移除**。勿重做同型。
//
// 原設計：手工列一張「舌象 → 親和方劑」表（7 個舌象各 6-8 個方），病人打得出舌象時
// 給表上的方在 score.total **之外**加一筆定額分（0.15～0.40，依症狀數分層）。
//
// 三個否決理由，前兩個是量測、第三個是硬傷：
//
// ① **對三個 battery 全部零影響**。掃了 6 組幅度（關閉 / 定額 0.15 / 定額 0.30 /
//    動態 .40-.30-.15 / .30-.20-.10 / .50-.35-.15），186 個案例（萩野 50＋大塚 114＋
//    臨床 22）的第1/前3/前5 **六組數字完全一樣**：萩野 8/13/18、大塚 39/68/81、
//    臨床 20/22/22。原因是結構性的——加分對親和表上的每個方**等量**加，整組一起
//    平移，組內相對順序不變；而正確答案本來就是自己那組的第一名（實測 #51 吳茱萸湯
//    0.747、#69 十全大補湯 0.952 在加分為 0 時就已經排第 1）。它唯一改變的是
//    第 2-5 名裡「非親和方」被親和方擠掉，而 battery 不量這個。
//
// ② 07-25／07-27 兩次 commit 宣稱的命中率提升（28%→34%）**不是這個機制帶來的**，
//    是同批進去的主症／詞彙補充帶來的——舌象詞彙一旦成為方的真正 keySymptoms，
//    證據就走正規計分路徑，這個外掛加分是重複計算。
//
// ③ **它把總分推破 1.0**，而前端顯示是 `Math.min(1, total) * 100`（x4-adapter.js
//    的 totalScore）。實測 #69（易疲勞/食欲不振/舌淡）在 0.40 幅度下有 3 個方
//    同時 >1.0，畫面上全部顯示 100%——鑑別度在 UI 上直接消失。
//
// 舌象要「優先」的正確做法不是加分，是**扣分**：病人給了舌象時，懲罰與該舌象
// 矛盾的方（例如舌淡的病人不該拿到清熱方）。那會真的改變排序，但需要一張
// 「舌象 × 方」的矛盾表，屬醫師層級的資料工程，不是參數。留給待辦。
//
// 真正落地的是舌診**詞彙**與**主症**（xlsx_to_kb.py 的 ONTOLOGY_ALIAS_PATCHES
// 舌診段＋KEY_SYMPTOM_PATCHES 的 吳茱萸湯/桂枝茯苓丸/十全大補湯/疏經活血湯），
// 那部分留著且有效（萩野前5 17→18、大塚前3 67→68）。
...
//
// **量測後否決（2026-07-17，第五個否決的方案，勿重試）：把主證加分的形狀廣義化到
// 所有方的所有主症命中**（不只 44 個有型宣告的方各自的那一個徵象）。
// 動機：待辦「覆蓋度分母」——大柴胡湯 vs 三黃瀉心湯 那類「被描述得完整卻因分母大
// 在近緣鑑別中吃虧」的一般形狀，主證加分只覆蓋 44 個方，沒有一般解法。
// 做法：加一個 W_KEY_HIT × min(1, positiveKeyHits/KEY_HIT_BONUS_K) 的加分，
// 與主證加分同型（加在路線 max 之上，不吃分母），掃了 W_KEY_HIT ∈ {0.05,0.10,0.15}
// × KEY_HIT_BONUS_K ∈ {2,3,4}。
// 結果：**閉閘（現況 KB）battery 含六經 20/22/22 全範圍不變，零回歸**——但也
// **零改善**，唯一有動的是無六經消融欄（20→21），不是使用者看得到的路線。
// 用 KAMPO_STRUCT_GATE=1 開閘量測（驗收工具，見 scripts/adjudicate_candidates.mjs）：
// 含六經 第1 18／前3 21／前5 22，比沒有這個加分時（18／21／22，即單靠主證加分）
// **完全沒有差異**——這個加分對開閘後的倒退沒有任何可量測的緩解效果。
// **結論**：把「加分不吃分母」這個形狀廣義化，本身沒有錯，也不會傷任何東西
// （這是它跟前四個方案的差異——那三個都會製造新的回歸），但它對開閘倒退這個
// 具體問題是惰性的——倒退的根因不是「大方的正確命中沒有出口」（那正是這個加分
// 想解決的），而是別的東西：一次升 300+ 筆主症時，分母膨脹是**全面性**的，
// 波及的形狀比任何一個方各自能拿到的、刻意封頂到很小的加分能抵銷的都大。
// 修法還是必須在別處——不是再調這個加分的權重或封頂（掃過的範圍已經是它能發揮
// 作用的全部空間，權重更大會開始製造新回歸，範圍內看不到這個問題的解）。
...
// ── 五臟軸也有分母問題，但**量測後不修**（2026-07-31，勿重試）─────────────
//
// 診斷（是真的，純算術沒有臨床歧義）：五臟向量的每一格 ＝ 命中數 ÷ **那一格自己的
// indicator 數**，而工作簿給各格列的徵象數從 4（心陽虛）到 10（腎陽虛）不等——
// **同樣的命中數，清單短的那一格分數高 2.5 倍**。而五臟分數走 cosine，各格尺度不齊
// 會讓「工作簿剛好列比較少徵象的那幾格」主導相似度。
// 真武湯＋腹滿 卡住的正是這個：它命中腎陽虛 4 個指標、脾氣虛 3 個——**證據比較支持
// 腎陽虛**——但 3/5 = 0.6 > 4/10 = 0.4，於是打破「SHEN-YANGXU ≥ PI-QIXU」哨兵。
//
// 修法試過：計分時把每格乘上 (自己的indicator數 / 平均數)，等價於十格共用一個分母。
// **量測後否決**：
//   per-state（現況）  萩 8/14/19 ｜ 塚 39/69/82 ｜ 臨 20/22/22 ｜ 測試綠
//   common（共同分母）  萩 7/14/19 ｜ 塚 40/69/81 ｜ 臨 **19**/22/22 ｜ **測試紅**
// 與待辦#2 第六個否決（特異度全域正規化）同一個結論：**尺度失真早就被既有調校吸收了，
// 事後校正它會擾動那 22 個教科書案。** 碼已移除。
//
// ⇒ 真武湯＋腹滿 目前無解，而且**不是靠拆 S-ABDOMINAL-DISTENSION 能解的**
//    （書自己不分水停腹脹與脾虛腹脹，見 SUSPENDED_KEY_SYMPTOM_PATCHES 註解）。
